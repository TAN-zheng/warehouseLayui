        ...
        <script>
                    ...
                    if (obj.event === 'del') {// obj.del();
                       ...
                       * */
                            $.ajax({
                                type: 'post',
                                //后台地址,提交后腰判断他是添加还是更改。
                                url: '${request.contextPath}/goods/deleteGoodsByIds',
                                //json方法,相当于是请求头
                        ...
                        layer.open({
                            ...
                            btn: ['确认', '取消'], //可以无限个按钮
                            //访问controller跳转页面。
                            content: ['${request.contextPath}/goods/changeGoodsLayer?id=' + data.id]//这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
                            //确认按钮点击的效果设置
                            , yes: function (index, layero) {
                                //按钮【按钮一】的回调
                                // alert("yes")
            ...
            * */
            ...
            window.reloadGoodsList = function () {
                table.reload("goods")//单引号
            }
